...
def load_storage_config(path: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise StorageConfigurationError(f'Invalid YAML configuration in {config_path}') from exc

    if not isinstance(data, dict):
        raise StorageConfigurationError(f'Configuration root must be a mapping in {config_path}')

    storage_config = data.get('storage')
    if not isinstance(storage_config, dict):
        raise StorageConfigurationError('Missing or invalid storage section in config.yaml')
...
def create_storage_repositories() -> tuple[object, object]:
    ...
    if backend == 'tinydb':
        ...
        if tinydb_config is not None and not isinstance(tinydb_config, dict):
            raise StorageConfigurationError('Invalid storage.tinydb section in config.yaml')
