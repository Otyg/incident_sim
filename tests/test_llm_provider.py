...
def test_get_llm_provider_defaults_to_ollama(monkeypatch):
    monkeypatch.setattr('src.services.llm_provider.CONFIG_PATH', Path('/tmp/test-config-default.yaml'))
    write_config(
    ...
    monkeypatch.setattr(OllamaProvider, '_create_client', staticmethod(lambda host, headers: object()))
...
def test_get_llm_provider_can_select_openai_stub(monkeypatch):
    monkeypatch.setattr('src.services.llm_provider.CONFIG_PATH', Path('/tmp/test-config-openai.yaml'))
    write_config(
...
def test_get_llm_provider_rejects_unknown_provider(monkeypatch):
    monkeypatch.setattr('src.services.llm_provider.CONFIG_PATH', Path('/tmp/test-config-unknown.yaml'))
    write_config(
...
def test_ollama_provider_uses_local_host_by_default(monkeypatch):
    ...
    monkeypatch.setattr(OllamaProvider, '_create_client', staticmethod(fake_create_client))
...
def test_ollama_provider_interpret_action_parses_json_response(monkeypatch):
    ...
    monkeypatch.setattr(OllamaProvider, '_create_client', staticmethod(lambda host, headers: FakeClient()))

    payload = OllamaProvider({'model': 'llama3.2'}).interpret_action('Vi samlar teamet.')
...
def test_load_llm_config_reads_provider_section(monkeypatch):
    monkeypatch.setattr('src.services.llm_provider.CONFIG_PATH', Path('/tmp/test-config-load.yaml'))
    write_config(
