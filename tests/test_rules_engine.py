...
def make_action(action_types: list[str], targets: list[str], summary: str) -> InterpretedAction:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            make_action(['communication'], ['media'], 'Vi går ut med ett första uttalande.'),
...
def test_rules_engine_updates_focus_items_for_core_rules(action, raw_input, expected_focus_item):
...
def test_rules_engine_updates_containment_effects():
    ...
    assert 'Begränsad extern åtkomst minskar attackytan men påverkar externa tjänster.' in updated.consequences


@pytest.mark.parametrize(
    ('starting_turns', 'expected_media_pressure', 'expected_public_confusion', 'expect_media_inject'),
    [
        (0, 1, 0, False),
        (1, 2, 1, True),
    ],
